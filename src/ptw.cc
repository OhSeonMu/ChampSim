...
PageTableWalker::PageTableWalker(Builder b)
    : champsim::operable(b.m_freq_scale), upper_levels(b.m_uls), lower_level(b.m_ll), NAME(b.m_name), MSHR_SIZE(b.m_mshr_size), MAX_READ(b.m_max_tag_check), MAX_FILL(b.m_max_fill), HIT_LATENCY(b.m_latency), vmem(b.m_vmem), CR3_addr(b.m_vmem->get_pte_pa(b.m_cpu, 0, b.m_vmem->pt_levels).first), 
      ...
      enable_asap(b.m_enable_asap), enable_ptempo(b.m_enable_ptempo), enable_calloc(b.m_enable_calloc), enable_coalescing(b.m_enable_coalescing), enable_bcoalescing(b.m_enable_bcoalescing), enable_abcoalescing(b.m_enable_abcoalescing), enable_aabcoalescing(b.m_enable_aabcoalescing), enable_aaabcoalescing(b.m_enable_aaabcoalescing)
{
...
auto PageTableWalker::handle_fill(const mshr_type& fill_mshr) -> std::optional<mshr_type>
{
  ...
  // TODO[OSM] : For miss prefetcher
  if(!fill_mshr.pf_metadata)
...
auto PageTableWalker::step_translation(mshr_type& source) -> std::optional<mshr_type>
{
  ...
  // TODO[OSM] : enable block coalescing 
    ...
    auto new_set_1 = (pd_offset & champsim::bitmask(champsim::lg2(page_in_super)));
    auto new_set_2 = pd_offset >> champsim::lg2(page_in_super);
    ...
    auto aab_pt_offset = pte_in_block * lage_set + block_set + page_in_super * pte_in_block * new_set_1;
    auto aaab_pt_offset = pte_in_block * lage_set + block_set + page_in_super * pte_in_block * new_set;
    auto new_pt_offset = 
	(enable_aaabcoalescing & enable_aabcoalescing & enable_abcoalescing) ? aaab_pt_offset :
...
void PageTableWalker::finish_packet(const response_type& packet)
{
  ...
  auto finish_last_step = [this](auto& mshr_entry) {
    uint64_t penalty;
    // TODO[OSM] : enable tlb coalescing
    if (this->enable_calloc)
      std::tie(mshr_entry.data, penalty) = this->vmem->va_to_pa_coalescing(mshr_entry.cpu, mshr_entry.v_address, enable_coalescing);
    else
