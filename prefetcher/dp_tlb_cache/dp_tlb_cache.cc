...
struct dp_cache {
  struct dp_entry {
    ...
    void prefetch_slot(uint64_t vpn, uint32_t metadata_in, CACHE* cache) const {
      for (const auto& d : lru_list) {
	uint64_t pf_vpn = vpn + (d << (LOG2_PAGE_SIZE + 3));
	if (!(cache->warmup))
	  cache->prefetch_line(pf_vpn, false, 11);
      }
    }
...
public:
  void initiate_lookahead(uint64_t vpn, uint32_t metadata_in, CACHE* cache) 
  {
    int64_t distance = ((static_cast<int64_t>(vpn) - static_cast<int64_t>(previous_vpn)) >> (LOG2_PAGE_SIZE + 3));
...
std::map<CACHE*, dp_cache> dps_cache;
}
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
  ...
  ::dps_cache[this].initiate_lookahead(addr, metadata_in, this);
  return metadata_in;
}
