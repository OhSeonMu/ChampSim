...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
  ...
  if (delta != 0) {
    pf_addr = addr + (delta << LOG2_PAGE_SIZE);
    if (!(this->warmup))
  ...
  delta = ::asps_cache[this].initiate_lookahead(ip, addr >> (LOG2_PAGE_SIZE + LOG2_PAGE_SIZE - (LOG2_BLOCK_SIZE + LOG2_PTE_SIZE)));
  if (delta != 0) {
    pf_addr = addr + (delta << (LOG2_PAGE_SIZE + (LOG2_PAGE_SIZE + LOG2_PAGE_SIZE - (LOG2_BLOCK_SIZE + LOG2_PTE_SIZE))));
    if (!(this->warmup))
