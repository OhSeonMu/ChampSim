...
{
  auto delta = ::asps[this].initiate_lookahead(ip, addr >> LOG2_BLOCK_SIZE);
  // metadata_in = 1;
  if (delta != 0) {
    uint64_t pf_addr = addr + (delta << LOG2_BLOCK_SIZE);
    // if (this->vmem->check_va_to_pa(this->cpu, pf_addr) & !warmup)
    // if (this->vmem->check_va_to_pa(this->cpu, pf_addr))
    if (!(this->warmup) && ((pf_addr >> LOG2_PAGE_SIZE) == (addr >> LOG2_PAGE_SIZE))) { 
