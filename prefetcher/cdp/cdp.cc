...
struct dp {
  struct dp_entry {
	...
	uint64_t pf_vpn = vpn + (d << LOG2_BLOCK_SIZE);
	// if (this->vmem->check_va_to_pa(this->cpu, pf_addr) & !warmup)
	if (!(cache->warmup) && ((pf_vpn >> LOG2_PAGE_SIZE) == (vpn >> LOG2_PAGE_SIZE)))
...
public:
  ...
  {
    int64_t distance = ((static_cast<int64_t>(vpn) - static_cast<int64_t>(previous_vpn)) >> LOG2_BLOCK_SIZE);
    if (distance < PAGE_SIZE / BLOCK_SIZE) {
      // current vpn  
