...
struct region_type {
  uint64_t vpn;
  std::bitset<(PAGE_SIZE / (BLOCK_SIZE * PTE_BYTES))> access_map{};
  std::bitset<(PAGE_SIZE / (BLOCK_SIZE * PTE_BYTES))> prefetch_map{};
  uint64_t lru;
...
auto pt_and_offset(uint64_t addr)
{
  ...
  auto page_offset = 
	  (addr & champsim::msl::bitmask(LOG2_PAGE_SIZE + champsim::lg2(PAGE_SIZE / PTE_BYTES) * (level - 1))) 
	  >> (champsim::lg2(PAGE_SIZE / (BLOCK_SIZE * PTE_BYTES)) + LOG2_PAGE_SIZE);
  return std::pair{page_number, page_offset};
}
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
  ...
  for (auto direction : {1, -1}) {
    for (int i = 1, prefetches_issued = 0; i <= MAX_DISTANCE && prefetches_issued < PREFETCH_DEGREE; i++) {
      const auto pos_step_addr = addr + direction * (i * (signed)(PAGE_SIZE / (BLOCK_SIZE * PTE_BYTES) + PAGE_SIZE));
      const auto neg_step_addr = addr - direction * (i * (signed)(PAGE_SIZE / (BLOCK_SIZE * PTE_BYTES) + PAGE_SIZE));
      const auto neg_2step_addr = addr - direction * (2 * i * (signed)(PAGE_SIZE / (BLOCK_SIZE * PTE_BYTES) + PAGE_SIZE));
      ...
      if (::check_cl_access(this, neg_step_addr) && ::check_cl_access(this, neg_2step_addr) && !::check_cl_access(this, pos_step_addr)
        ...
        if ((addr >> (champsim::lg2(PAGE_SIZE / (BLOCK_SIZE * PTE_BYTES)) + LOG2_PAGE_SIZE )) != 
			(pos_step_addr >> (champsim::lg2(PAGE_SIZE / (BLOCK_SIZE * PTE_BYTES)) + LOG2_PAGE_SIZE)) {
          bool prefetch_success = prefetch_line(pos_step_addr, 0, 11);
          if (prefetch_success) {
            auto [pf_vpn, pf_page_offset] = ::pt_and_offset(pos_step_addr);
